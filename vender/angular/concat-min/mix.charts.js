...
(function($, R){
    ...
    function calcGrid(scope, xlables) {
	    ...
	    if(barwidth > 40 * me.data.length) {
	    	me.box.barwidth = 35 * me.data.length;
	    }else {
  ...
  Mix.bar = bar;
	...
	Pie.prototype = {
	    ...
	    calcBox: function() {
			...
			box.r = Math.min(box.cx, box.cy) - opts.strokeWidth - 20;
	...
	//          'stroke-width': opts.strokeWidth
		...
		drawLabel: function(labelTxt, paper, cx, cy, r, startAngle, endAngle, color) {
			 var x1 = cx + r * Math.cos(-(startAngle+endAngle)/2 * rad),
			      ...
			      x2 = cx + (r+8) * Math.cos(-(startAngle+endAngle)/2 * rad),
			      y2 = cy + (r+8) * Math.sin(-(startAngle+endAngle)/2 * rad),
			      x3, y3;
