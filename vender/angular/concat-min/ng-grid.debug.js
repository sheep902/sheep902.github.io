...
angular.module('ngGrid.services').factory('$domUtilityService',['$utilityService', '$window', function($utils, $window) {
    ...
    domUtilityService.getRealWidth = function (obj) {
        ...
        $.swap(hiddenParents[0], props, function () {
            hiddenParents[0].style['display']="block";
...
var ngFooter = function ($scope, grid) {
    ...
    $scope.multiSelect = grid.config.multiSelect;
...
ngRow.prototype.setSelection = function (isSelected,notPush) {
    this.selectionProvider.setSelection(this, isSelected,notPush);
...
var ngRowFactory = function (grid, $scope, domUtilityService, $templateCache, $utils) {
    ...
    self.UpdateViewableRange = function(newRange) {
        ...
        if($scope.renderedRows.length===0){
            grid.$viewport[0].style.height ="0px";
            grid.$root[0].style.height=$scope.topPanelHeight()+ $scope.footerRowHeight+1 + "px";
        }
    };
...
var ngSelectionProvider = function (grid, $scope, $parse) {
    ...
    self.setSelection = function (rowItem, isSelected,notPush) {
...
var ngStyleProvider = function($scope, grid) {
    ...
    $scope.rowStyle = function (row) {
        ...
        if (grid.rowCache[row.rowIndex + 1] != null){
            ...
            grid.$viewport[0].style.height = row.offsetTop + rowHeight+1 + "px";
            grid.$root[0].style.height=row.offsetTop + rowHeight + $scope.topPanelHeight()+ $scope.footerRowHeight+2 + "px";
        }
        ...
        compile: function () {
            return {
                pre: function ($scope, iElement) {
                    ...
                    // }
                    if (iElement.children().length === 0) {
                        var maxRows=$scope.maxRows(),pageSize=$scope.pagingOptions.pageSize;
                        $scope.ngPager=new AM.Pager({
                            container:iElement,
                            type:$scope.pagingOptions.pageType||2,
                            showTotal:$scope.pagingOptions.showTotal||2,
                            showTotalType:$scope.pagingOptions.showTotalType||2,
                            count:maxRows,
                            i18n:$scope.pagingOptions.i18n,
                            pageSize:pageSize,//每页条数
                            page:Math.ceil(maxRows / pageSize),
                            onPager:function(num,self){
                                $scope.$apply(function(){
                                    $scope.pagingOptions.currentPage=num;
                                });
                            }
                        });
                    }
                ...
                pre: function($scope, iElement, iAttrs) {
                    ...
                    return grid.init().then(function() {
                        ...
                        if (typeof options.data === "string") {
                            ...
                            $scope.$parent.$watchCollection(options.data, dataWatcher);
                        ...
                        options.selectRow = function (rowIndex, state,notPush) {
                            if (grid.rowCache[rowIndex]) {
                                if (grid.rowCache[rowIndex].clone) {
                                    grid.rowCache[rowIndex].clone.setSelection(state ? true : false,notPush);
                                } 
                                grid.rowCache[rowIndex].setSelection(state ? true : false,notPush);
                            }
                        };
                        options.selectItem = function (itemIndex, state,notPush) {
                            options.selectRow(grid.rowMap[itemIndex], state,notPush);
                        };
...
angular.module("ngGrid").run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put("checkboxCellTemplate.html",
    "<div class=\"ngSelectionCell\"><input tabindex=\"-1\" class=\"ngSelectionCheckbox\" type=\"checkbox\" ng-checked=\"row.selected\" ng-disabled='checkboxDisabled' /></div>"
  );

  $templateCache.put("checkboxHeaderTemplate.html",
    "<input class=\"ngSelectionHeader\" type=\"checkbox\" ng-show=\"multiSelect\" ng-model=\"allSelected\" ng-disabled='checkboxDisabled' ng-change=\"toggleSelectAll(allSelected, true)\"/>"
  );
...
//  );
    $templateCache.put("editableCellTemplate.html",
        "<div class=\"ngCellText\"><input class=\"form-control\" style=\"width:100%\" ng-input=\"COL_FIELD\" ng-model=\"COL_FIELD\" /></div>"
    );
  ...
  $templateCache.put("gridTemplate.html",
    ...
    "    </div>\n" +
    "</div>\n" +
    "<div ng-grid-footer ng-show=\"enablePaging\"></div>\n"
  );

  $templateCache.put("headerCellTemplate.html",
    "<div class=\"ngHeaderSortColumn {{col.headerClass}}\" ng-style=\"{'cursor': col.cursor}\" ng-class=\"{ 'ngSorted': !noSortVisible }\">\n" +
    "    <div ng-click=\"col.sort($event)\" ng-class=\"'colt' + col.index\" class=\"ngHeaderText\"><p>{{col.displayName}}</p></div>\n" +
        ...
        if (rowScope) {
            ...
            if (event.target.type !== 'checkbox') {
                ...
                if (!row.selected) {
                    ...
                    setTimeout(function(){
                        row.selectionProvider.setSelection(row, false);
                        self.scope.$apply();
                    },100);
                }
            }
